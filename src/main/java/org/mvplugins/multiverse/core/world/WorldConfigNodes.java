...
final class WorldConfigNodes {
    ...
    final ConfigNode<Boolean> allowWeather = node(ConfigNode.builder("allow-weather", Boolean.class)
            ...
            .onLoadAndChange((oldValue, newValue) -> {
                if (!(world instanceof LoadedMultiverseWorld loadedWorld)) return;
                loadedWorld.getBukkitWorld().peek(w -> {
                    Runnable weatherTask = () -> {
                        if (!w.isClearWeather() && !newValue) {
                            w.setThundering(false);
                            w.setStorm(false);
                        }
                    };
                    if (org.mvplugins.multiverse.core.folia.FoliaDetector.isFolia()) {
    ...
    final ConfigNode<Difficulty> difficulty = node(ConfigNode.builder("difficulty", Difficulty.class)
            ...
            .onLoadAndChange((oldValue, newValue) -> {
                ...
                loadedWorld.getBukkitWorld().peek(bukkitWorld -> {
                    Runnable difficultyTask = () -> bukkitWorld.setDifficulty(newValue);
                    if (org.mvplugins.multiverse.core.folia.FoliaDetector.isFolia()) {
                        org.mvplugins.multiverse.core.folia.FoliaSchedulerAdapter.runGlobalTask(
                                Bukkit.getPluginManager().getPlugin("Multiverse-Core"),
                                difficultyTask
    ...
    final ConfigNode<Boolean> pvp = node(ConfigNode.builder("pvp", Boolean.class)
            ...
            .onLoadAndChange((oldValue, newValue) -> {
                if (!(world instanceof LoadedMultiverseWorld loadedWorld)) return;
                loadedWorld.getBukkitWorld().peek(bukkitWorld -> {
                    Runnable pvpTask = () -> bukkitWorld.setPVP(newValue);
                    if (org.mvplugins.multiverse.core.folia.FoliaDetector.isFolia()) {
                        org.mvplugins.multiverse.core.folia.FoliaSchedulerAdapter.runGlobalTask(
                                Bukkit.getPluginManager().getPlugin("Multiverse-Core"),
                                pvpTask
